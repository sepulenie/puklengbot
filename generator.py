    ...
    for word_0, word_1 in pair_of_words:
        ...
        if search_result == None:
            ...
            if word_1 in search_result_word_1.keys():
                search_result_word_1.update({word_1: (search_result_word_1.get(word_1)+100)})
...
def generate_message(message, chat_id):
    ...
    message = re.sub(r"\n", " ", message)
    words_in_message = message.split()                                           
    random_case = random.randint(1,1)  


    if random_case == 0:                                                            
        sentence_lengh = random.randint(2, 50)                                      
        ...
        for i in range(sentence_lengh):
            ...
            if search_result == None:
                ...
                random_next_word = random.choices(keys_list, cum_weights=values_list, k=1)
        ...
        for i in range(sentences_amount):
            ...
            for i in range(sentences_amount):
                ...
                if search_result == None:
                    ...
                    values_list = list(search_result_as_dict.values())
                    random_next_word = random.choices(keys_list, cum_weights=values_list, k=1)
                    if random_next_word == '.' or random_next_word == '?' or random_next_word == '!':
                        sentence.append(random_next_word[0])
                        current_word_in_sentence = random_next_word[0]
                        break
                    else:
